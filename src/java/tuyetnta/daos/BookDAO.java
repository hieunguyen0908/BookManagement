...
public class BookDAO implements Serializable {
    ...
    public List<BookDTO> search(String title, Integer min, Integer max, String category) throws SQLException, NamingException {
        ...
        try {
            ...
            String sql = "SELECT b.id , b.title ,b.image ,b.describe ,b.price ,b.author ,b.importDate ,b.quantity ,c.name AS categoryName FROM dbo.books AS b JOIN dbo.categorys AS c ON c.id = b.cateID WHERE (b.title LIKE ? COLLATE Latin1_General_CI_AI) AND b.status = 'active' AND (? IS NULL OR c.name = ?) AND (? IS NULL OR b.price >= ?) AND (? IS NULL OR b.price <= ?) AND b.quantity > 0";
