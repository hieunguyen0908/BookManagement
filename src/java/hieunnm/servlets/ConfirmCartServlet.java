...
public class ConfirmCartServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Map<String, BookDTO> invalidBookQuantity = cart.checkQuantity();
            if (invalidBookQuantity.isEmpty()) {
                if (cart.confirm()) {
                    request.setAttribute("MSG_SUCCESS", "Confirm cart success");
                     url = SEARCH_PAGE;
                } else {
                ...
                String invalidErr = "";
                for (Map.Entry<String, BookDTO> e : invalidBookQuantity.entrySet()) {
                    BookDTO bo = e.getValue();
                    if (bo.getQuantity() > 0) {
                        invalidErr += e.getValue().getTitle() + " has only " + e.getValue().getQuantity() + "\n";
                    } else if (bo.getQuantity() == 0) {
                        invalidErr += e.getValue().getTitle() + " is out of stock\n";
                    } else {
                        invalidErr += e.getValue().getTitle() + " not exist\n";
                    }
                }
